		...
		<div>
			<nav className='flex justify-between w-full items-center p-2 md:p-4  dark:bg-transparent md:bg-transparent shadow-md md:shadow-none sticky md:relative top-0'>
				...
				<div className=' flex justify-end md:justify-center gap-8 p-2 w-full md:w-fit items-center'>
					<ThemeToggle />
					<IoNotificationsOutline className='h-[1.2rem] w-[1.2rem] hover:scale-110 text-ui_primary dark:text-white hover:text-ui_secondary1 font-[700] duration-500 ease-in-out cursor-pointer' />
					{/* Customer icon */}
					<Link
						href='/auth/dashboard/Accounts'
						className='flex gap-3 flex-row-reverse items-center'
					>
						{user ? (
						...
						<span className={`flex flex-col gap-0 md:hidden`}>
							<h3 className='text-md font-[600] text-ui_primary'>
								...
								{''} <br />
								<span className='text-[0.77rem] dark:text-dark_text'>({user?.email})</span>
							</h3>
